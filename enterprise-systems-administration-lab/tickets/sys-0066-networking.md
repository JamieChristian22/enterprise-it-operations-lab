# SYS-0066: Duplicate IP address detected
...
| Affected asset | FS01 |
| Opened | 2026-07-15 02:22 ET |
| First response | 2026-07-15 02:31 ET |
| Resolved | 2026-07-15 03:19 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-051 – Resolve Duplicate IP Addresses |

## Executive Summary

Duplicate IP address detected affected Operations operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 02:22 ET, Riley Johnson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-15 02:22 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-15 02:31 ET:** Jamie Christian accepted ownership, assigned the ticket to Network Operations, and began diagnostics.
- **2026-07-15 02:43 ET:** Initial tests isolated the fault to the Networking service layer on or affecting FS01.
- **2026-07-15 03:07 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-15 03:19 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `arp -a`
2. `Get-DhcpServerv4Lease -IPAddress 10.20.30.45`
3. `ping -a 10.20.30.45`

### Diagnostic Findings

- `arp -a` → Command output matched the expected post-remediation configuration.
- `Get-DhcpServerv4Lease -IPAddress 10.20.30.45` → Command output matched the expected post-remediation configuration.
- `ping -a 10.20.30.45` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **A printer retained a static address that had been reintroduced into the DHCP dynamic pool.**

## Root Cause

A printer retained a static address that had been reintroduced into the DHCP dynamic pool.
...
Moved the printer to a documented reservation outside the dynamic range, cleared neighbor caches, and updated inventory.
...
- `SYS-0066-01-initial-alert.txt` — original user or monitoring report
- `SYS-0066-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0066-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-051 – Resolve Duplicate IP Addresses**.
...
Resolved SYS-0066 after isolating the failure to the Networking service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
