# SYS-0069: Security baseline drift detected
...
| Opened | 2026-07-15 17:43 ET |
| First response | 2026-07-15 17:52 ET |
| Resolved | 2026-07-15 18:33 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-075 – Remediate Security Configuration Drift |

## Executive Summary

Security baseline drift detected affected Engineering operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 17:43 ET, Sydney Clark reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-15 17:43 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-15 17:52 ET:** Jamie Christian accepted ownership, assigned the ticket to Security Operations, and began diagnostics.
- **2026-07-15 18:04 ET:** Initial tests isolated the fault to the Security service layer on or affecting SQL01.
- **2026-07-15 18:21 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-15 18:33 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `secedit /export /cfg C:\Temp\local-policy.inf`
2. `auditpol /get /category:*`
3. `gpresult /h C:\Temp\gp.html`

### Diagnostic Findings

- `secedit /export /cfg C:\Temp\local-policy.inf` → Command output matched the expected post-remediation configuration.
- `auditpol /get /category:*` → Command output matched the expected post-remediation configuration.
- `gpresult /h C:\Temp\gp.html` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **A server administrator applied a local policy that overrode the approved audit configuration.**

## Root Cause

A server administrator applied a local policy that overrode the approved audit configuration.
...
Removed the local override, re-applied the baseline GPO, and confirmed settings through compliance scanning.
...
- `SYS-0069-01-initial-alert.txt` — original user or monitoring report
- `SYS-0069-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0069-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-075 – Remediate Security Configuration Drift**.
...
Resolved SYS-0069 after isolating the failure to the Security service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
