# SYS-0090: Slow file transfers between VLANs
...
| Affected asset | FS01 |
| Opened | 2026-07-20 02:40 ET |
| First response | 2026-07-20 02:49 ET |
| Resolved | 2026-07-20 03:37 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-053 – Diagnose Network Performance |

## Executive Summary

Slow file transfers between VLANs affected Operations operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 02:40 ET, Drew Wilson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-20 02:40 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-20 02:49 ET:** Jamie Christian accepted ownership, assigned the ticket to Network Operations, and began diagnostics.
- **2026-07-20 03:01 ET:** Initial tests isolated the fault to the Networking service layer on or affecting FS01.
- **2026-07-20 03:25 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-20 03:37 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-NetAdapter | Select Name,Status,LinkSpeed`
2. `Get-NetAdapterStatistics`
3. `ping -f -l 1472 FS02`

### Diagnostic Findings

- `Get-NetAdapter | Select Name,Status,LinkSpeed` → Command output matched the expected post-remediation configuration.
- `Get-NetAdapterStatistics` → Command output matched the expected post-remediation configuration.
- `ping -f -l 1472 FS02` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **One server NIC negotiated at 100 Mbps full duplex after a cable fault.**

## Root Cause

One server NIC negotiated at 100 Mbps full duplex after a cable fault.
...
Replaced the damaged cable, confirmed 1 Gbps negotiation, and repeated the transfer test within baseline.
...
- `SYS-0090-01-initial-alert.txt` — original user or monitoring report
- `SYS-0090-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0090-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-053 – Diagnose Network Performance**.
...
Resolved SYS-0090 after isolating the failure to the Networking service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
