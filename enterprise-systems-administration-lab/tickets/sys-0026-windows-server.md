# SYS-0026: Remote Desktop unavailable
...
| Affected asset | FS01 |
| Opened | 2026-07-06 18:42 ET |
| First response | 2026-07-06 18:51 ET |
| Resolved | 2026-07-06 19:39 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-015 – Restore Remote Desktop Connectivity |

## Executive Summary

Remote Desktop unavailable affected Operations operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 18:42 ET, Riley Johnson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-06 18:42 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-06 18:51 ET:** Jamie Christian accepted ownership, assigned the ticket to Windows Platform Operations, and began diagnostics.
- **2026-07-06 19:03 ET:** Initial tests isolated the fault to the Windows Server service layer on or affecting FS01.
- **2026-07-06 19:27 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-06 19:39 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Test-NetConnection APP01 -Port 3389`
2. `Get-NetFirewallRule -DisplayGroup 'Remote Desktop'`
3. `Get-Service TermService`

### Diagnostic Findings

- `Test-NetConnection APP01 -Port 3389` → Connectivity test returned `TcpTestSucceeded: True` after remediation.
- `Get-NetFirewallRule -DisplayGroup 'Remote Desktop'` → Command output matched the expected post-remediation configuration.
- `Get-Service TermService` → Service state reported Running/active and remained stable through the observation period.

The collected evidence showed that **The Windows Firewall RDP rule was disabled after a local security template was applied manually.**

## Root Cause

The Windows Firewall RDP rule was disabled after a local security template was applied manually.
...
Re-enabled the domain-profile RDP rules through Group Policy, restarted Remote Desktop Services, and validated from the management subnet.
...
- `SYS-0026-01-initial-alert.txt` — original user or monitoring report
- `SYS-0026-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0026-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-015 – Restore Remote Desktop Connectivity**.
...
Resolved SYS-0026 after isolating the failure to the Windows Server service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
