# SYS-0089: Virtual host time drift
...
| Department | Finance |
| Affected asset | DC01 |
| Opened | 2026-07-19 21:33 ET |
| First response | 2026-07-19 21:42 ET |
| Resolved | 2026-07-19 22:23 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-045 – Configure Virtualized Domain Controller Time |

## Executive Summary

Virtual host time drift affected Finance operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 21:33 ET, Sydney Clark reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-19 21:33 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-19 21:42 ET:** Jamie Christian accepted ownership, assigned the ticket to Virtualization and Compute, and began diagnostics.
- **2026-07-19 21:54 ET:** Initial tests isolated the fault to the Virtualization service layer on or affecting DC01.
- **2026-07-19 22:11 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-19 22:23 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-VMIntegrationService -VMName DC02`
2. `w32tm /query /source`
3. `w32tm /query /status`

### Diagnostic Findings

- `Get-VMIntegrationService -VMName DC02` → Command output matched the expected post-remediation configuration.
- `w32tm /query /source` → Command output matched the expected post-remediation configuration.
- `w32tm /query /status` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **A domain controller VM synchronized from both the Hyper-V host and the domain time hierarchy.**

## Root Cause

A domain controller VM synchronized from both the Hyper-V host and the domain time hierarchy.
...
Disabled Hyper-V time synchronization for the domain controller guest and confirmed synchronization from the domain hierarchy.
...
- `SYS-0089-01-initial-alert.txt` — original user or monitoring report
- `SYS-0089-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0089-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-045 – Configure Virtualized Domain Controller Time**.
...
Resolved SYS-0089 after isolating the failure to the Virtualization service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
