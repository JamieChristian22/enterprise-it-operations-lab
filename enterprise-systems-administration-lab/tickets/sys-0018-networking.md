# SYS-0018: DHCP clients receive APIPA addresses
...
| Affected asset | FS01 |
| Opened | 2026-07-05 02:36 ET |
| First response | 2026-07-05 02:41 ET |
| Resolved | 2026-07-05 03:42 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-047 – Resolve DHCP Address Assignment |

## Executive Summary

DHCP clients receive APIPA addresses affected Operations operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 02:36 ET, Cameron Lewis reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-05 02:36 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-05 02:41 ET:** Jamie Christian accepted ownership, assigned the ticket to Network Operations, and began diagnostics.
- **2026-07-05 02:53 ET:** Initial tests isolated the fault to the Networking service layer on or affecting FS01.
- **2026-07-05 03:30 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-05 03:42 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-DhcpServerv4Scope`
2. `Get-DhcpServerv4ScopeStatistics`
3. `Get-DhcpServerv4Lease -ScopeId 10.40.10.0`
4. `ipconfig /renew`

### Diagnostic Findings

- `Get-DhcpServerv4Scope` → Command output matched the expected post-remediation configuration.
- `Get-DhcpServerv4ScopeStatistics` → Command output matched the expected post-remediation configuration.
- `Get-DhcpServerv4Lease -ScopeId 10.40.10.0` → Command output matched the expected post-remediation configuration.
- `ipconfig /renew` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The DHCP scope was exhausted after temporary training devices consumed the remaining leases.**

## Root Cause

The DHCP scope was exhausted after temporary training devices consumed the remaining leases.
...
Removed expired temporary reservations, expanded the approved scope range, reconciled the database, and renewed affected clients.
...
- `SYS-0018-01-initial-alert.txt` — original user or monitoring report
- `SYS-0018-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0018-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-047 – Resolve DHCP Address Assignment**.
...
Resolved SYS-0018 after isolating the failure to the Networking service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
