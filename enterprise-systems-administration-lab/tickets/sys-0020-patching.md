# SYS-0020: Patch caused application regression
...
| Opened | 2026-07-05 12:50 ET |
| First response | 2026-07-05 12:55 ET |
| Resolved | 2026-07-05 13:42 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-063 – Respond to Patch Regressions |

## Executive Summary

Patch caused application regression affected Sales operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 12:50 ET, Drew Wilson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-05 12:50 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-05 12:55 ET:** Jamie Christian accepted ownership, assigned the ticket to Patch and Vulnerability Management, and began diagnostics.
- **2026-07-05 13:07 ET:** Initial tests isolated the fault to the Patching service layer on or affecting WEB02.
- **2026-07-05 13:30 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-05 13:42 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-HotFix | Sort InstalledOn -Descending`
2. `Get-WinEvent -LogName Application -MaxEvents 100`
3. `Test-NetConnection LEGACYDB01 -Port 1433`
...
- `Test-NetConnection LEGACYDB01 -Port 1433` → Connectivity test returned `TcpTestSucceeded: True` after remediation.

The collected evidence showed that **A .NET cumulative update changed TLS defaults used by a legacy internal application.**

## Root Cause

A .NET cumulative update changed TLS defaults used by a legacy internal application.
...
Rolled back the approved update under emergency change, restored service, and documented the vendor remediation requirement.
...
- `SYS-0020-01-initial-alert.txt` — original user or monitoring report
- `SYS-0020-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0020-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-063 – Respond to Patch Regressions**.
...
Resolved SYS-0020 after isolating the failure to the Patching service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
