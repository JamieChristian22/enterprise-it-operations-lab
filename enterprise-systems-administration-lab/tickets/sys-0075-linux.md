# SYS-0075: sudo access missing for operations engineer
...
| Opened | 2026-07-16 23:35 ET |
| First response | 2026-07-16 23:53 ET |
| Resolved | 2026-07-17 00:27 ET |
| SLA target | 4-business-hour response / 2-business-day resolution |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-025 – Administer Linux Privileged Access |

## Executive Summary

sudo access missing for operations engineer affected Human Resources operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 23:35 ET, Casey Patel reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-16 23:35 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-16 23:53 ET:** Jamie Christian accepted ownership, assigned the ticket to Linux Platform Operations, and began diagnostics.
- **2026-07-17 00:05 ET:** Initial tests isolated the fault to the Linux service layer on or affecting APP01.
- **2026-07-17 00:15 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-17 00:27 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `id jrivera`
2. `getent group linux-ops`
3. `sss_cache -E`
4. `sudo -l -U jrivera`

### Diagnostic Findings

- `id jrivera` → Command output matched the expected post-remediation configuration.
- `getent group linux-ops` → Command output matched the expected post-remediation configuration.
- `sss_cache -E` → Command output matched the expected post-remediation configuration.
- `sudo -l -U jrivera` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The user was added to the directory group but SSSD had cached the previous group membership.**

## Root Cause

The user was added to the directory group but SSSD had cached the previous group membership.
...
Cleared the SSSD cache, restarted SSSD during the approved window, and verified command-specific sudo access.
...
- `SYS-0075-01-initial-alert.txt` — original user or monitoring report
- `SYS-0075-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0075-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-025 – Administer Linux Privileged Access**.
...
Resolved SYS-0075 after isolating the failure to the Linux service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
