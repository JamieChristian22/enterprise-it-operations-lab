# SYS-0054: Required application port blocked
...
| Department | Legal |
| Affected asset | HV01 |
| Opened | 2026-07-12 14:38 ET |
| First response | 2026-07-12 14:47 ET |
| Resolved | 2026-07-12 15:35 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-050 – Troubleshoot Network Ports and Firewalls |

## Executive Summary

Required application port blocked affected Legal operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 14:38 ET, Morgan Chen reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-12 14:38 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-12 14:47 ET:** Jamie Christian accepted ownership, assigned the ticket to Network Operations, and began diagnostics.
- **2026-07-12 14:59 ET:** Initial tests isolated the fault to the Networking service layer on or affecting HV01.
- **2026-07-12 15:23 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-12 15:35 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Test-NetConnection APP04 -Port 8443`
2. `Get-NetFirewallRule -PolicyStore ActiveStore`
3. `Get-NetTCPConnection -LocalPort 8443`

### Diagnostic Findings

- `Test-NetConnection APP04 -Port 8443` → Connectivity test returned `TcpTestSucceeded: True` after remediation.
- `Get-NetFirewallRule -PolicyStore ActiveStore` → Command output matched the expected post-remediation configuration.
- `Get-NetTCPConnection -LocalPort 8443` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The destination server was moved to a hardened OU whose firewall policy did not include the application rule.**

## Root Cause

The destination server was moved to a hardened OU whose firewall policy did not include the application rule.
...
Added a source-restricted firewall rule through Group Policy, refreshed policy, and validated the application transaction.
...
- `SYS-0054-01-initial-alert.txt` — original user or monitoring report
- `SYS-0054-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0054-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-050 – Troubleshoot Network Ports and Firewalls**.
...
Resolved SYS-0054 after isolating the failure to the Networking service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
