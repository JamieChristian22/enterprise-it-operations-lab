# SYS-0062: SMB share unavailable after reboot
...
| Department | Legal |
| Affected asset | HV01 |
| Opened | 2026-07-14 06:44 ET |
| First response | 2026-07-14 06:53 ET |
| Resolved | 2026-07-14 07:41 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-018 – Restore SMB File Shares |

## Executive Summary

SMB share unavailable after reboot affected Legal operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 06:44 ET, Jordan Rivera reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-14 06:44 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-14 06:53 ET:** Jamie Christian accepted ownership, assigned the ticket to Windows Platform Operations, and began diagnostics.
- **2026-07-14 07:05 ET:** Initial tests isolated the fault to the Windows Server service layer on or affecting HV01.
- **2026-07-14 07:29 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-14 07:41 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-SmbShare`
2. `Get-Volume`
3. `Get-WinEvent -LogName System -MaxEvents 100 | Where-Object Id -in 55,98,2012`

### Diagnostic Findings

- `Get-SmbShare` → Command output matched the expected post-remediation configuration.
- `Get-Volume` → Capacity returned to a healthy operating range and remained below the warning threshold.
- `Get-WinEvent -LogName System -MaxEvents 100 | Where-Object Id -in 55,98,2012` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.

The collected evidence showed that **The data volume mounted after the Server service initialized, leaving the share path unavailable.**

## Root Cause

The data volume mounted after the Server service initialized, leaving the share path unavailable.
...
Corrected the storage dependency, recreated the share with documented permissions, and tested access from two client subnets.
...
- `SYS-0062-01-initial-alert.txt` — original user or monitoring report
- `SYS-0062-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0062-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-018 – Restore SMB File Shares**.
...
Resolved SYS-0062 after isolating the failure to the Windows Server service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
