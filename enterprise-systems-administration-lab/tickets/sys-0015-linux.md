# SYS-0015: Root filesystem at 98 percent
...
| Opened | 2026-07-04 11:15 ET |
| First response | 2026-07-04 11:20 ET |
| Resolved | 2026-07-04 12:28 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-021 – Resolve Linux Disk Capacity |

## Executive Summary

Root filesystem at 98 percent affected Marketing operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 11:15 ET, Casey Patel reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-04 11:15 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-04 11:20 ET:** Jamie Christian accepted ownership, assigned the ticket to Linux Platform Operations, and began diagnostics.
- **2026-07-04 11:32 ET:** Initial tests isolated the fault to the Linux service layer on or affecting MON01.
- **2026-07-04 12:16 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-04 12:28 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `df -hT`
2. `du -xhd1 /var | sort -h`
3. `find /var/log/app -type f -size +500M -ls`
4. `lsof +L1`

### Diagnostic Findings

- `df -hT` → Capacity returned to a healthy operating range and remained below the warning threshold.
- `du -xhd1 /var | sort -h` → Command output matched the expected post-remediation configuration.
- `find /var/log/app -type f -size +500M -ls` → Command output matched the expected post-remediation configuration.
- `lsof +L1` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **A runaway application generated uncompressed debug logs under /var/log/app.**

## Root Cause

A runaway application generated uncompressed debug logs under /var/log/app.
...
Stopped debug logging, compressed and archived retained logs, removed expired files, and added logrotate limits.
...
- `SYS-0015-01-initial-alert.txt` — original user or monitoring report
- `SYS-0015-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0015-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-021 – Resolve Linux Disk Capacity**.
...
Resolved SYS-0015 after isolating the failure to the Linux service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
