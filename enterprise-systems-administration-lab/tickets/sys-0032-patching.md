# SYS-0032: Linux security updates pending beyond SLA
...
| Opened | 2026-07-08 00:34 ET |
| First response | 2026-07-08 00:43 ET |
| Resolved | 2026-07-08 01:17 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-064 – Patch Linux Servers |

## Executive Summary

Linux security updates pending beyond SLA affected Customer Success operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 00:34 ET, Jordan Rivera reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.

## Business Impact

The issue disrupted a production workflow used by the Customer Success department. Based on affected users, service criticality, and the availability of temporary alternatives, the incident was assigned **P2** with **high impact** and **medium urgency**.
...
- **2026-07-08 00:34 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-08 00:43 ET:** Jamie Christian accepted ownership, assigned the ticket to Patch and Vulnerability Management, and began diagnostics.
- **2026-07-08 00:55 ET:** Initial tests isolated the fault to the Patching service layer on or affecting BKP01.
- **2026-07-08 01:05 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-08 01:17 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `dnf check-update --security`
2. `dnf repolist`
3. `dnf repoquery --unsatisfied`
4. `dnf history`

### Diagnostic Findings

- `dnf check-update --security` → Command output matched the expected post-remediation configuration.
- `dnf repolist` → Command output matched the expected post-remediation configuration.
- `dnf repoquery --unsatisfied` → Command output matched the expected post-remediation configuration.
- `dnf history` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **A dependency conflict from a retired third-party repository blocked the patch transaction.**

## Root Cause

A dependency conflict from a retired third-party repository blocked the patch transaction.
...
Disabled the retired repository, removed the deprecated agent, installed security updates, and rebooted during the maintenance window.
...
- `SYS-0032-01-initial-alert.txt` — original user or monitoring report
- `SYS-0032-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0032-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-064 – Patch Linux Servers**.
...
Resolved SYS-0032 after isolating the failure to the Patching service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
