# SYS-0094: Quota notification not delivered
...
| Department | Legal |
| Affected asset | HV01 |
| Opened | 2026-07-20 22:18 ET |
| First response | 2026-07-20 22:36 ET |
| Resolved | 2026-07-20 23:03 ET |
| SLA target | 4-business-hour response / 2-business-day resolution |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-084 – Configure Storage Quotas and Alerts |

## Executive Summary

Quota notification not delivered affected Legal operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 22:18 ET, Morgan Chen reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.

## Business Impact

The issue disrupted a production workflow used by the Legal department. Based on affected users, service criticality, and the availability of temporary alternatives, the incident was assigned **P3** with **low impact** and **low urgency**.
...
- **2026-07-20 22:18 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-20 22:36 ET:** Jamie Christian accepted ownership, assigned the ticket to Storage and File Services, and began diagnostics.
- **2026-07-20 22:48 ET:** Initial tests isolated the fault to the Storage service layer on or affecting HV01.
- **2026-07-20 22:51 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-20 23:03 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-FsrmSetting`
2. `Get-FsrmQuota`
3. `Test-NetConnection mailrelay.corp.example.com -Port 587`

### Diagnostic Findings

- `Get-FsrmSetting` → Command output matched the expected post-remediation configuration.
- `Get-FsrmQuota` → Command output matched the expected post-remediation configuration.
- `Test-NetConnection mailrelay.corp.example.com -Port 587` → Connectivity test returned `TcpTestSucceeded: True` after remediation.

The collected evidence showed that **FSRM email notifications still referenced a retired SMTP relay.**

## Root Cause

FSRM email notifications still referenced a retired SMTP relay.
...
Updated FSRM SMTP settings, sent a test notification, and verified threshold alerts.
...
- `SYS-0094-01-initial-alert.txt` — original user or monitoring report
- `SYS-0094-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0094-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-084 – Configure Storage Quotas and Alerts**.
...
Resolved SYS-0094 after isolating the failure to the Storage service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
