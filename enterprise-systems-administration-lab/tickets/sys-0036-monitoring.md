# SYS-0036: Disk alert failed to trigger
...
| Opened | 2026-07-08 20:12 ET |
| First response | 2026-07-08 20:17 ET |
| Resolved | 2026-07-08 21:04 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-095 – Validate Monitoring Coverage |

## Executive Summary

Disk alert failed to trigger affected Sales operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 20:12 ET, Riley Johnson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-08 20:12 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-08 20:17 ET:** Jamie Christian accepted ownership, assigned the ticket to Monitoring and Event Management, and began diagnostics.
- **2026-07-08 20:29 ET:** Initial tests isolated the fault to the Monitoring service layer on or affecting WEB02.
- **2026-07-08 20:52 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-08 21:04 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-Volume`
2. `Get-Service MonitorAgent`
3. `Get-WinEvent -LogName Application -MaxEvents 100 | Where-Object Message -match 'discovery'`
...
- `Get-WinEvent -LogName Application -MaxEvents 100 | Where-Object Message -match 'discovery'` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.

The collected evidence showed that **The new data volume was not included in the monitor's discovery filter.**

## Root Cause

The new data volume was not included in the monitor's discovery filter.
...
Updated volume discovery, forced rediscovery, tested the threshold safely, and confirmed alert routing.
...
- `SYS-0036-01-initial-alert.txt` — original user or monitoring report
- `SYS-0036-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0036-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-095 – Validate Monitoring Coverage**.
...
Resolved SYS-0036 after isolating the failure to the Monitoring service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
