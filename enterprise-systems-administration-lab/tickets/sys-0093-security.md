# SYS-0093: Privileged access review overdue
...
| Opened | 2026-07-20 17:11 ET |
| First response | 2026-07-20 17:20 ET |
| Resolved | 2026-07-20 18:01 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-040 – Privileged Access Review |

## Executive Summary

Privileged access review overdue affected Engineering operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 17:11 ET, Taylor Brooks reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-20 17:11 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-20 17:20 ET:** Jamie Christian accepted ownership, assigned the ticket to Security Operations, and began diagnostics.
- **2026-07-20 17:32 ET:** Initial tests isolated the fault to the Security service layer on or affecting SQL01.
- **2026-07-20 17:49 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-20 18:01 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-ADGroupMember Domain-Server-Admins -Recursive`
2. `Get-ADUser tempadmin1 -Properties AccountExpirationDate,LastLogonDate`
3. `Get-MgRoleManagementDirectoryRoleAssignment`

### Diagnostic Findings

- `Get-ADGroupMember Domain-Server-Admins -Recursive` → Command output matched the expected post-remediation configuration.
- `Get-ADUser tempadmin1 -Properties AccountExpirationDate,LastLogonDate` → Command output matched the expected post-remediation configuration.
- `Get-MgRoleManagementDirectoryRoleAssignment` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **Two temporary project administrators retained membership beyond the approved expiration date.**

## Root Cause

Two temporary project administrators retained membership beyond the approved expiration date.
...
Removed expired access, obtained owner certification for remaining members, and updated the quarterly access-review record.
...
- `SYS-0093-01-initial-alert.txt` — original user or monitoring report
- `SYS-0093-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0093-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-040 – Privileged Access Review**.
...
Resolved SYS-0093 after isolating the failure to the Security service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
