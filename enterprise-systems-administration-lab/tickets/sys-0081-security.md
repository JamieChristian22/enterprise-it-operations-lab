# SYS-0081: Unencrypted protocol found in use
...
| Department | Finance |
| Affected asset | DC01 |
| Opened | 2026-07-18 05:27 ET |
| First response | 2026-07-18 05:36 ET |
| Resolved | 2026-07-18 06:17 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-076 – Remove Insecure Protocols |

## Executive Summary

Unencrypted protocol found in use affected Finance operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 05:27 ET, Alex Martin reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-18 05:27 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-18 05:36 ET:** Jamie Christian accepted ownership, assigned the ticket to Security Operations, and began diagnostics.
- **2026-07-18 05:48 ET:** Initial tests isolated the fault to the Security service layer on or affecting DC01.
- **2026-07-18 06:05 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-18 06:17 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Test-NetConnection mailrelay.corp.example.com -Port 25`
2. `Get-NetTCPConnection -RemotePort 25`
3. `Get-MessageTrace -SenderAddress scanner@corp.example.com`

### Diagnostic Findings

- `Test-NetConnection mailrelay.corp.example.com -Port 25` → Connectivity test returned `TcpTestSucceeded: True` after remediation.
- `Get-NetTCPConnection -RemotePort 25` → Command output matched the expected post-remediation configuration.
...
The collected evidence showed that **A multifunction printer was sending scans through unauthenticated SMTP on port 25.**

## Root Cause

A multifunction printer was sending scans through unauthenticated SMTP on port 25.
...
Moved the printer to the authenticated TLS relay, disabled legacy relay access, and validated secure message delivery.
...
- `SYS-0081-01-initial-alert.txt` — original user or monitoring report
- `SYS-0081-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0081-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-076 – Remove Insecure Protocols**.
...
Resolved SYS-0081 after isolating the failure to the Security service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
