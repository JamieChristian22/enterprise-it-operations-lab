# SYS-0078: VPN tunnel connects but resources unavailable
...
| Department | Legal |
| Affected asset | HV01 |
| Opened | 2026-07-17 14:56 ET |
| First response | 2026-07-17 15:05 ET |
| Resolved | 2026-07-17 15:53 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-052 – Troubleshoot Remote Access VPN |

## Executive Summary

VPN tunnel connects but resources unavailable affected Legal operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 14:56 ET, Cameron Lewis reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-17 14:56 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-17 15:05 ET:** Jamie Christian accepted ownership, assigned the ticket to Network Operations, and began diagnostics.
- **2026-07-17 15:17 ET:** Initial tests isolated the fault to the Networking service layer on or affecting HV01.
- **2026-07-17 15:41 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-17 15:53 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `route print`
2. `ipconfig /all`
3. `Test-NetConnection 10.80.10.25 -Port 443`
4. `tracert 10.80.10.25`

### Diagnostic Findings

- `route print` → Command output matched the expected post-remediation configuration.
- `ipconfig /all` → Command output matched the expected post-remediation configuration.
- `Test-NetConnection 10.80.10.25 -Port 443` → Connectivity test returned `TcpTestSucceeded: True` after remediation.
- `tracert 10.80.10.25` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The client received a VPN address but lacked a route for the newly created server subnet.**

## Root Cause

The client received a VPN address but lacked a route for the newly created server subnet.
...
Added the approved subnet to the VPN split-tunnel route list, reconnected the client, and validated DNS and application access.
...
- `SYS-0078-01-initial-alert.txt` — original user or monitoring report
- `SYS-0078-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0078-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-052 – Troubleshoot Remote Access VPN**.
...
Resolved SYS-0078 after isolating the failure to the Networking service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
