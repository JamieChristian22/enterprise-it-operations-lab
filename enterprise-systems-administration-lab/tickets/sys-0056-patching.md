# SYS-0056: Patch deployment missed isolated servers
...
| Opened | 2026-07-13 00:52 ET |
| First response | 2026-07-13 01:01 ET |
| Resolved | 2026-07-13 01:35 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-066 – Troubleshoot Patch Management Connectivity |

## Executive Summary

Patch deployment missed isolated servers affected Customer Success operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 00:52 ET, Riley Johnson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-13 00:52 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-13 01:01 ET:** Jamie Christian accepted ownership, assigned the ticket to Patch and Vulnerability Management, and began diagnostics.
- **2026-07-13 01:13 ET:** Initial tests isolated the fault to the Patching service layer on or affecting BKP01.
- **2026-07-13 01:23 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-13 01:35 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Test-NetConnection WSUS01 -Port 8531`
2. `Get-WindowsUpdateLog`
3. `gpresult /r`

### Diagnostic Findings

- `Test-NetConnection WSUS01 -Port 8531` → Connectivity test returned `TcpTestSucceeded: True` after remediation.
- `Get-WindowsUpdateLog` → Command output matched the expected post-remediation configuration.
- `gpresult /r` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The new management subnet was absent from the WSUS firewall allow list.**

## Root Cause

The new management subnet was absent from the WSUS firewall allow list.
...
Added the management subnet to the restricted WSUS rule, refreshed policy, triggered update detection, and verified compliance reporting.
...
- `SYS-0056-01-initial-alert.txt` — original user or monitoring report
- `SYS-0056-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0056-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-066 – Troubleshoot Patch Management Connectivity**.
...
Resolved SYS-0056 after isolating the failure to the Patching service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
