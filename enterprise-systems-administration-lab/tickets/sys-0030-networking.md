# SYS-0030: Internal DNS record resolves incorrectly
...
| Department | Legal |
| Affected asset | HV01 |
| Opened | 2026-07-07 14:20 ET |
| First response | 2026-07-07 14:29 ET |
| Resolved | 2026-07-07 15:17 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-048 – Correct DNS Records |

## Executive Summary

Internal DNS record resolves incorrectly affected Legal operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 14:20 ET, Drew Wilson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-07 14:20 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-07 14:29 ET:** Jamie Christian accepted ownership, assigned the ticket to Network Operations, and began diagnostics.
- **2026-07-07 14:41 ET:** Initial tests isolated the fault to the Networking service layer on or affecting HV01.
- **2026-07-07 15:05 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-07 15:17 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Resolve-DnsName app01.corp.example.com`
2. `Get-DnsServerResourceRecord -ZoneName corp.example.com -Name app01`
3. `ipconfig /flushdns`

### Diagnostic Findings

- `Resolve-DnsName app01.corp.example.com` → Authoritative and client lookups returned the documented production address.
- `Get-DnsServerResourceRecord -ZoneName corp.example.com -Name app01` → Command output matched the expected post-remediation configuration.
- `ipconfig /flushdns` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **A stale static A record remained after the application server migrated to a new address.**

## Root Cause

A stale static A record remained after the application server migrated to a new address.
...
Updated the authoritative record, incremented the zone, cleared resolver caches, and verified from all DNS servers.
...
- `SYS-0030-01-initial-alert.txt` — original user or monitoring report
- `SYS-0030-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0030-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-048 – Correct DNS Records**.
...
Resolved SYS-0030 after isolating the failure to the Networking service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
