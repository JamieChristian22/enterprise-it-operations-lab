# SYS-0061: Service account password expired
...
| Opened | 2026-07-14 01:37 ET |
| First response | 2026-07-14 01:46 ET |
| Resolved | 2026-07-14 02:27 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-009 – Maintain Service Accounts |

## Executive Summary

Service account password expired affected Engineering operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 01:37 ET, Alex Martin reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-14 01:37 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-14 01:46 ET:** Jamie Christian accepted ownership, assigned the ticket to Identity and Access Administration, and began diagnostics.
- **2026-07-14 01:58 ET:** Initial tests isolated the fault to the Active Directory service layer on or affecting SQL01.
- **2026-07-14 02:15 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-14 02:27 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-ADUser svc_reports -Properties PasswordExpired,PasswordLastSet,PasswordNeverExpires`
2. `Get-ScheduledTask -TaskName 'Nightly Finance Export'`
3. `Get-WinEvent -FilterHashtable @{LogName='Security';Id=4625} -MaxEvents 25`

### Diagnostic Findings

- `Get-ADUser svc_reports -Properties PasswordExpired,PasswordLastSet,PasswordNeverExpires` → Command output matched the expected post-remediation configuration.
- `Get-ScheduledTask -TaskName 'Nightly Finance Export'` → Command output matched the expected post-remediation configuration.
- `Get-WinEvent -FilterHashtable @{LogName='Security';Id=4625} -MaxEvents 25` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.

The collected evidence showed that **A legacy scheduled task used a standard domain user account with password expiration enabled.**

## Root Cause

A legacy scheduled task used a standard domain user account with password expiration enabled.
...
Rotated the password through the approved vault, updated the scheduled task credential, tested the job, and opened a change to migrate to gMSA.
...
- `SYS-0061-01-initial-alert.txt` — original user or monitoring report
- `SYS-0061-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0061-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-009 – Maintain Service Accounts**.
...
Resolved SYS-0061 after isolating the failure to the Active Directory service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
