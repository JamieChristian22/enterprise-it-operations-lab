# SYS-0024: CPU threshold alert is false positive
...
| Opened | 2026-07-06 08:28 ET |
| First response | 2026-07-06 08:46 ET |
| Resolved | 2026-07-06 08:59 ET |
| SLA target | 4-business-hour response / 2-business-day resolution |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-094 – Tune Monitoring Alerts |

## Executive Summary

CPU threshold alert is false positive affected Customer Success operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 08:28 ET, Morgan Chen reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.

## Business Impact

The issue disrupted a production workflow used by the Customer Success department. Based on affected users, service criticality, and the availability of temporary alternatives, the incident was assigned **P3** with **low impact** and **low urgency**.
...
- **2026-07-06 08:28 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-06 08:46 ET:** Jamie Christian accepted ownership, assigned the ticket to Monitoring and Event Management, and began diagnostics.
- **2026-07-06 08:58 ET:** Initial tests isolated the fault to the Monitoring service layer on or affecting BKP01.
- **2026-07-06 08:47 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-06 08:59 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-Counter '\Processor(_Total)\% Processor Time' -SampleInterval 5 -MaxSamples 12`
2. `Get-MpComputerStatus`

### Diagnostic Findings

- `Get-Counter '\Processor(_Total)\% Processor Time' -SampleInterval 5 -MaxSamples 12` → Command output matched the expected post-remediation configuration.
- `Get-MpComputerStatus` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The monitor sampled a one-minute antivirus scan spike and required five consecutive breaches for meaningful alerting.**

## Root Cause

The monitor sampled a one-minute antivirus scan spike and required five consecutive breaches for meaningful alerting.
...
Adjusted the rule to require five consecutive samples above threshold and documented the approved exception.
...
- `SYS-0024-01-initial-alert.txt` — original user or monitoring report
- `SYS-0024-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0024-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-094 – Tune Monitoring Alerts**.
...
Resolved SYS-0024 after isolating the failure to the Monitoring service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
