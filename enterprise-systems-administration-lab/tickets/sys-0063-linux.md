# SYS-0063: High CPU on application server
...
| Opened | 2026-07-14 11:51 ET |
| First response | 2026-07-14 12:00 ET |
| Resolved | 2026-07-14 12:55 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-024 – Investigate Linux Performance |

## Executive Summary

High CPU on application server affected Marketing operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 11:51 ET, Taylor Brooks reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-14 11:51 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-14 12:00 ET:** Jamie Christian accepted ownership, assigned the ticket to Linux Platform Operations, and began diagnostics.
- **2026-07-14 12:12 ET:** Initial tests isolated the fault to the Linux service layer on or affecting MON01.
- **2026-07-14 12:43 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-14 12:55 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.
...
3. `pidstat 1 5`
4. `ss -plant`
5. `journalctl -u orders-api --since '1 hour ago'`

### Diagnostic Findings

- `uptime` → Command output matched the expected post-remediation configuration.
- `top -b -n1 | head -30` → Command output matched the expected post-remediation configuration.
- `pidstat 1 5` → Command output matched the expected post-remediation configuration.
- `ss -plant` → Command output matched the expected post-remediation configuration.
- `journalctl -u orders-api --since '1 hour ago'` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.

The collected evidence showed that **A Java process entered a retry loop after losing connectivity to its message broker.**

## Root Cause

A Java process entered a retry loop after losing connectivity to its message broker.
...
Restored broker connectivity, restarted the application gracefully, and added dependency-health monitoring.
...
- `SYS-0063-01-initial-alert.txt` — original user or monitoring report
- `SYS-0063-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0063-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-024 – Investigate Linux Performance**.
...
Resolved SYS-0063 after isolating the failure to the Linux service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
