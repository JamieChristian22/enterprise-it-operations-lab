# SYS-0050: Windows time drift exceeds policy
...
| Affected asset | FS01 |
| Opened | 2026-07-11 18:10 ET |
| First response | 2026-07-11 18:19 ET |
| Resolved | 2026-07-11 19:07 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-017 – Correct Windows Time Synchronization |

## Executive Summary

Windows time drift exceeds policy affected Operations operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 18:10 ET, Drew Wilson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-11 18:10 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-11 18:19 ET:** Jamie Christian accepted ownership, assigned the ticket to Windows Platform Operations, and began diagnostics.
- **2026-07-11 18:31 ET:** Initial tests isolated the fault to the Windows Server service layer on or affecting FS01.
- **2026-07-11 18:55 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-11 19:07 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.
...
- `w32tm /monitor` → Command output matched the expected post-remediation configuration.
- `netdom query fsmo` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The PDC emulator was configured to use a retired internal NTP appliance.**

## Root Cause

The PDC emulator was configured to use a retired internal NTP appliance.
...
Configured the PDC emulator with approved external NTP peers, restarted W32Time, and forced domain hierarchy resynchronization.
...
- `SYS-0050-01-initial-alert.txt` — original user or monitoring report
- `SYS-0050-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0050-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-017 – Correct Windows Time Synchronization**.
...
Resolved SYS-0050 after isolating the failure to the Windows Server service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
