# SYS-0016: Shared mailbox access denied
...
| Opened | 2026-07-04 16:22 ET |
| First response | 2026-07-04 16:40 ET |
| Resolved | 2026-07-04 16:53 ET |
| SLA target | 4-business-hour response / 2-business-day resolution |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-031 – Manage Shared Mailbox Permissions |

## Executive Summary

Shared mailbox access denied affected Customer Success operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 16:22 ET, Riley Johnson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-04 16:22 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-04 16:40 ET:** Jamie Christian accepted ownership, assigned the ticket to Microsoft 365 Administration, and began diagnostics.
- **2026-07-04 16:52 ET:** Initial tests isolated the fault to the Microsoft 365 service layer on or affecting BKP01.
- **2026-07-04 16:41 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-04 16:53 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-MailboxPermission ap@corp.example.com`
2. `Get-RecipientPermission ap@corp.example.com`
3. `Get-DistributionGroupMember AP-SharedMailbox-Users`

### Diagnostic Findings

- `Get-MailboxPermission ap@corp.example.com` → Command output matched the expected post-remediation configuration.
- `Get-RecipientPermission ap@corp.example.com` → Command output matched the expected post-remediation configuration.
- `Get-DistributionGroupMember AP-SharedMailbox-Users` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **Full Access was assigned through a group that had not synchronized to Exchange Online.**

## Root Cause

Full Access was assigned through a group that had not synchronized to Exchange Online.
...
Assigned approved Full Access and Send As permissions directly, verified propagation, and opened a follow-up to correct group synchronization.
...
- `SYS-0016-01-initial-alert.txt` — original user or monitoring report
- `SYS-0016-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0016-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-031 – Manage Shared Mailbox Permissions**.
...
Resolved SYS-0016 after isolating the failure to the Microsoft 365 service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
