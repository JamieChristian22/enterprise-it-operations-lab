# SYS-0041: Hyper-V live migration fails
...
| Department | Finance |
| Affected asset | DC01 |
| Opened | 2026-07-09 21:47 ET |
| First response | 2026-07-09 21:56 ET |
| Resolved | 2026-07-09 22:37 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-041 – Resolve Live Migration Failures |

## Executive Summary

Hyper-V live migration fails affected Finance operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 21:47 ET, Alex Martin reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-09 21:47 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-09 21:56 ET:** Jamie Christian accepted ownership, assigned the ticket to Virtualization and Compute, and began diagnostics.
- **2026-07-09 22:08 ET:** Initial tests isolated the fault to the Virtualization service layer on or affecting DC01.
- **2026-07-09 22:25 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-09 22:37 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-VMHost | Select VirtualMachineMigrationEnabled,VirtualMachineMigrationAuthenticationType`
2. `setspn -Q cifs/HV02`
3. `Get-ADComputer HV01 -Properties msDS-AllowedToDelegateTo`

### Diagnostic Findings

- `Get-VMHost | Select VirtualMachineMigrationEnabled,VirtualMachineMigrationAuthenticationType` → Command output matched the expected post-remediation configuration.
- `setspn -Q cifs/HV02` → Command output matched the expected post-remediation configuration.
- `Get-ADComputer HV01 -Properties msDS-AllowedToDelegateTo` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **Kerberos constrained delegation was missing for the destination host's CIFS service.**

## Root Cause

Kerberos constrained delegation was missing for the destination host's CIFS service.
...
Updated constrained delegation through an approved change, refreshed Kerberos tickets, and completed a test migration.
...
- `SYS-0041-01-initial-alert.txt` — original user or monitoring report
- `SYS-0041-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0041-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-041 – Resolve Live Migration Failures**.
...
Resolved SYS-0041 after isolating the failure to the Virtualization service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
