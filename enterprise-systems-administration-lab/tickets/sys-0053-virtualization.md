# SYS-0053: VM performance degraded
...
| Opened | 2026-07-12 09:31 ET |
| First response | 2026-07-12 09:40 ET |
| Resolved | 2026-07-12 10:21 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-042 – Diagnose Virtual Machine Performance |

## Executive Summary

VM performance degraded affected Engineering operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 09:31 ET, Taylor Brooks reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-12 09:31 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-12 09:40 ET:** Jamie Christian accepted ownership, assigned the ticket to Virtualization and Compute, and began diagnostics.
- **2026-07-12 09:52 ET:** Initial tests isolated the fault to the Virtualization service layer on or affecting SQL01.
- **2026-07-12 10:09 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-12 10:21 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-Counter '\Hyper-V Virtual Storage Device(*)\Average Read Time'`
2. `Get-Volume`
3. `Get-VMHardDiskDrive`

### Diagnostic Findings

- `Get-Counter '\Hyper-V Virtual Storage Device(*)\Average Read Time'` → Command output matched the expected post-remediation configuration.
- `Get-Volume` → Capacity returned to a healthy operating range and remained below the warning threshold.
- `Get-VMHardDiskDrive` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **Storage latency increased because four VMs shared a thin-provisioned volume at 92-percent utilization.**

## Root Cause

Storage latency increased because four VMs shared a thin-provisioned volume at 92-percent utilization.
...
Moved the busiest VHDX to the performance tier, expanded the volume, and confirmed disk latency returned below the alert threshold.
...
- `SYS-0053-01-initial-alert.txt` — original user or monitoring report
- `SYS-0053-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0053-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-042 – Diagnose Virtual Machine Performance**.
...
Resolved SYS-0053 after isolating the failure to the Virtualization service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
