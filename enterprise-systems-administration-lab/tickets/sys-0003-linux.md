# SYS-0003: SSH connections rejected
...
| Opened | 2026-07-01 23:31 ET |
| First response | 2026-07-01 23:40 ET |
| Resolved | 2026-07-02 00:35 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-017 – Troubleshoot Linux SSH Failures |

## Executive Summary

SSH connections rejected affected Human Resources operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 23:31 ET, Taylor Brooks reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-01 23:31 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-01 23:40 ET:** Jamie Christian accepted ownership, assigned the ticket to Linux Platform Operations, and began diagnostics.
- **2026-07-01 23:52 ET:** Initial tests isolated the fault to the Linux service layer on or affecting APP01.
- **2026-07-02 00:23 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-02 00:35 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `systemctl status sshd`
2. `sshd -t`
3. `journalctl -u sshd --since '30 minutes ago'`
4. `getent group linux-admins`

### Diagnostic Findings

- `systemctl status sshd` → Service state reported Running/active and remained stable through the observation period.
- `sshd -t` → Command output matched the expected post-remediation configuration.
- `journalctl -u sshd --since '30 minutes ago'` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.
- `getent group linux-admins` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **A configuration-management change restricted AllowGroups but omitted the linux-admins group.**

## Root Cause

A configuration-management change restricted AllowGroups but omitted the linux-admins group.
...
Added linux-admins to the approved AllowGroups setting, validated syntax, reloaded sshd, and tested key-based login.
...
- `SYS-0003-01-initial-alert.txt` — original user or monitoring report
- `SYS-0003-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0003-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-017 – Troubleshoot Linux SSH Failures**.
...
Resolved SYS-0003 after isolating the failure to the Linux service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
