# SYS-0040: Teams client repeatedly prompts for sign-in
...
| Opened | 2026-07-09 16:40 ET |
| First response | 2026-07-09 16:58 ET |
| Resolved | 2026-07-09 17:11 ET |
| SLA target | 4-business-hour response / 2-business-day resolution |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-033 – Resolve Microsoft 365 Authentication |

## Executive Summary

Teams client repeatedly prompts for sign-in affected Customer Success operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 16:40 ET, Drew Wilson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-09 16:40 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-09 16:58 ET:** Jamie Christian accepted ownership, assigned the ticket to Microsoft 365 Administration, and began diagnostics.
- **2026-07-09 17:10 ET:** Initial tests isolated the fault to the Microsoft 365 service layer on or affecting BKP01.
- **2026-07-09 16:59 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-09 17:11 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `dsregcmd /status`
2. `cmdkey /list`
3. `Get-MgAuditLogSignIn -Top 20`
...
- `Get-MgAuditLogSignIn -Top 20` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **Corrupted Web Account Manager tokens remained after the user's password reset.**

## Root Cause

Corrupted Web Account Manager tokens remained after the user's password reset.
...
Signed out of Office, cleared approved token caches and stale credentials, restarted the device, and completed modern authentication.
...
- `SYS-0040-01-initial-alert.txt` — original user or monitoring report
- `SYS-0040-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0040-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-033 – Resolve Microsoft 365 Authentication**.
...
Resolved SYS-0040 after isolating the failure to the Microsoft 365 service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
