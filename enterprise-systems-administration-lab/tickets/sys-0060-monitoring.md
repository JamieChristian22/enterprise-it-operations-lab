# SYS-0060: Alert notifications not reaching on-call
...
| Opened | 2026-07-13 20:30 ET |
| First response | 2026-07-13 20:35 ET |
| Resolved | 2026-07-13 21:22 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-097 – Troubleshoot Alert Notifications |

## Executive Summary

Alert notifications not reaching on-call affected Sales operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 20:30 ET, Drew Wilson reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-13 20:30 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-13 20:35 ET:** Jamie Christian accepted ownership, assigned the ticket to Monitoring and Event Management, and began diagnostics.
- **2026-07-13 20:47 ET:** Initial tests isolated the fault to the Monitoring service layer on or affecting WEB02.
- **2026-07-13 21:10 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-13 21:22 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Test-NetConnection notify.corp.example.com -Port 443`
2. `Get-WinEvent -LogName Application -MaxEvents 100 | Where-Object Message -match 'notification'`

### Diagnostic Findings

- `Test-NetConnection notify.corp.example.com -Port 443` → Connectivity test returned `TcpTestSucceeded: True` after remediation.
- `Get-WinEvent -LogName Application -MaxEvents 100 | Where-Object Message -match 'notification'` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.

The collected evidence showed that **The notification workflow referenced an expired webhook secret.**

## Root Cause

The notification workflow referenced an expired webhook secret.
...
Rotated the webhook secret in the vault and monitoring platform, sent a test P1 alert, and confirmed on-call receipt.
...
- `SYS-0060-01-initial-alert.txt` — original user or monitoring report
- `SYS-0060-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0060-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-097 – Troubleshoot Alert Notifications**.
...
Resolved SYS-0060 after isolating the failure to the Monitoring service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
