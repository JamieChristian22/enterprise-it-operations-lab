# SYS-0051: Cron backup job stopped running
...
| Opened | 2026-07-11 23:17 ET |
| First response | 2026-07-11 23:26 ET |
| Resolved | 2026-07-12 00:21 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-023 – Troubleshoot Scheduled Linux Jobs |

## Executive Summary

Cron backup job stopped running affected Human Resources operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 23:17 ET, Alex Martin reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-11 23:17 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-11 23:26 ET:** Jamie Christian accepted ownership, assigned the ticket to Linux Platform Operations, and began diagnostics.
- **2026-07-11 23:38 ET:** Initial tests isolated the fault to the Linux service layer on or affecting APP01.
- **2026-07-12 00:09 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-12 00:21 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `crontab -l`
2. `journalctl -u cron --since yesterday`
3. `run-parts --test /etc/cron.daily`
4. `shellcheck /opt/scripts/db-backup.sh`

### Diagnostic Findings

- `crontab -l` → Command output matched the expected post-remediation configuration.
- `journalctl -u cron --since yesterday` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.
- `run-parts --test /etc/cron.daily` → Command output matched the expected post-remediation configuration.
- `shellcheck /opt/scripts/db-backup.sh` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The script depended on a relative executable path unavailable in cron's limited environment.**

## Root Cause

The script depended on a relative executable path unavailable in cron's limited environment.
...
Converted all paths to absolute values, set a controlled PATH, added exit-code logging, and completed a successful manual and scheduled run.
...
- `SYS-0051-01-initial-alert.txt` — original user or monitoring report
- `SYS-0051-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0051-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-023 – Troubleshoot Scheduled Linux Jobs**.
...
Resolved SYS-0051 after isolating the failure to the Linux service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
