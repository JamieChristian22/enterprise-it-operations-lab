# SYS-0037: User cannot authenticate after legal name change
...
| Opened | 2026-07-09 01:19 ET |
| First response | 2026-07-09 01:37 ET |
| Resolved | 2026-07-09 01:57 ET |
| SLA target | 4-business-hour response / 2-business-day resolution |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-006 – Update User Identity Attributes |

## Executive Summary

User cannot authenticate after legal name change affected Engineering operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 01:19 ET, Avery Davis reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-09 01:19 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-09 01:37 ET:** Jamie Christian accepted ownership, assigned the ticket to Identity and Access Administration, and began diagnostics.
- **2026-07-09 01:49 ET:** Initial tests isolated the fault to the Active Directory service layer on or affecting SQL01.
- **2026-07-09 01:45 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-09 01:57 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-ADUser tbrooks -Properties UserPrincipalName,SamAccountName,ProxyAddresses`
2. `klist`
3. `cmdkey /list`

### Diagnostic Findings

- `Get-ADUser tbrooks -Properties UserPrincipalName,SamAccountName,ProxyAddresses` → Command output matched the expected post-remediation configuration.
- `klist` → Command output matched the expected post-remediation configuration.
- `cmdkey /list` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The user's UPN was updated but the cached Windows profile and Outlook credential still referenced the previous sign-in name.**

## Root Cause

The user's UPN was updated but the cached Windows profile and Outlook credential still referenced the previous sign-in name.
...
Confirmed the new UPN and proxy addresses, removed cached credentials, purged Kerberos tickets, and completed a sign-out/sign-in cycle.
...
- `SYS-0037-01-initial-alert.txt` — original user or monitoring report
- `SYS-0037-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0037-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-006 – Update User Identity Attributes**.
...
Resolved SYS-0037 after isolating the failure to the Active Directory service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
