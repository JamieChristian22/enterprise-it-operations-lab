# SYS-0014: Server disk space critically low
...
| Department | Legal |
| Affected asset | HV01 |
| Opened | 2026-07-04 06:58 ET |
| First response | 2026-07-04 07:03 ET |
| Resolved | 2026-07-04 08:04 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-014 – Remediate Windows Server Disk Capacity |

## Executive Summary

Server disk space critically low affected Legal operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 06:58 ET, Morgan Chen reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-04 06:58 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-04 07:03 ET:** Jamie Christian accepted ownership, assigned the ticket to Windows Platform Operations, and began diagnostics.
- **2026-07-04 07:15 ET:** Initial tests isolated the fault to the Windows Server service layer on or affecting HV01.
- **2026-07-04 07:52 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-04 08:04 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-Volume`
2. `Get-ChildItem C:\inetpub\logs -Recurse | Sort-Object Length -Descending | Select-Object -First 20`
3. `Get-Counter '\LogicalDisk(C:)\% Free Space'`

### Diagnostic Findings

- `Get-Volume` → Capacity returned to a healthy operating range and remained below the warning threshold.
- `Get-ChildItem C:\inetpub\logs -Recurse | Sort-Object Length -Descending | Select-Object -First 20` → Command output matched the expected post-remediation configuration.
- `Get-Counter '\LogicalDisk(C:)\% Free Space'` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **Verbose IIS logging and failed-request traces accumulated for six months on the system volume.**

## Root Cause

Verbose IIS logging and failed-request traces accumulated for six months on the system volume.
...
Archived required logs, deleted expired traces under retention policy, enabled log rollover, and configured an 80-percent capacity alert.
...
- `SYS-0014-01-initial-alert.txt` — original user or monitoring report
- `SYS-0014-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0014-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-014 – Remediate Windows Server Disk Capacity**.
...
Resolved SYS-0014 after isolating the failure to the Windows Server service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
