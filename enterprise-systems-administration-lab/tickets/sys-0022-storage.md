# SYS-0022: NTFS access denied after department transfer
...
| Department | Legal |
| Affected asset | HV01 |
| Opened | 2026-07-05 22:14 ET |
| First response | 2026-07-05 22:23 ET |
| Resolved | 2026-07-05 23:11 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-078 – Troubleshoot File Permissions |

## Executive Summary

NTFS access denied after department transfer affected Legal operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 22:14 ET, Jordan Rivera reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-05 22:14 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-05 22:23 ET:** Jamie Christian accepted ownership, assigned the ticket to Storage and File Services, and began diagnostics.
- **2026-07-05 22:35 ET:** Initial tests isolated the fault to the Storage service layer on or affecting HV01.
- **2026-07-05 22:59 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-05 23:11 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-Acl D:\Shares\Operations`
2. `Get-ADPrincipalGroupMembership jrivera`
3. `icacls D:\Shares\Operations`

### Diagnostic Findings

- `Get-Acl D:\Shares\Operations` → Command output matched the expected post-remediation configuration.
- `Get-ADPrincipalGroupMembership jrivera` → Command output matched the expected post-remediation configuration.
- `icacls D:\Shares\Operations` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The user retained the old department group but was missing the new resource group.**

## Root Cause

The user retained the old department group but was missing the new resource group.
...
Obtained data-owner approval, corrected group membership, refreshed the token, and validated least-privilege access.
...
- `SYS-0022-01-initial-alert.txt` — original user or monitoring report
- `SYS-0022-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0022-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-078 – Troubleshoot File Permissions**.
...
Resolved SYS-0022 after isolating the failure to the Storage service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
