# SYS-0072: Service health check flapping
...
| Opened | 2026-07-16 08:14 ET |
| First response | 2026-07-16 08:23 ET |
| Resolved | 2026-07-16 08:57 ET |
| SLA target | 30-minute response / 8-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-098 – Design Reliable Health Checks |

## Executive Summary

Service health check flapping affected Customer Success operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 08:14 ET, Jordan Rivera reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-16 08:14 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-16 08:23 ET:** Jamie Christian accepted ownership, assigned the ticket to Monitoring and Event Management, and began diagnostics.
- **2026-07-16 08:35 ET:** Initial tests isolated the fault to the Monitoring service layer on or affecting BKP01.
- **2026-07-16 08:45 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-16 08:57 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Invoke-WebRequest https://reports.corp.example.com/health -UseBasicParsing`
2. `Test-NetConnection reports.corp.example.com -Port 443`

### Diagnostic Findings

- `Invoke-WebRequest https://reports.corp.example.com/health -UseBasicParsing` → Command output matched the expected post-remediation configuration.
- `Test-NetConnection reports.corp.example.com -Port 443` → Connectivity test returned `TcpTestSucceeded: True` after remediation.

The collected evidence showed that **The HTTP monitor used a five-second timeout while the application normally reached seven seconds during report generation.**

## Root Cause

The HTTP monitor used a five-second timeout while the application normally reached seven seconds during report generation.
...
Created a lightweight health endpoint, updated the monitor to use it, and confirmed stable checks without hiding real failures.
...
- `SYS-0072-01-initial-alert.txt` — original user or monitoring report
- `SYS-0072-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0072-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-098 – Design Reliable Health Checks**.
...
Resolved SYS-0072 after isolating the failure to the Monitoring service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
