# SYS-0034: Storage volume marked offline
...
| Affected asset | FS01 |
| Opened | 2026-07-08 10:48 ET |
| First response | 2026-07-08 10:53 ET |
| Resolved | 2026-07-08 11:54 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-079 – Recover Offline Storage |

## Executive Summary

Storage volume marked offline affected Operations operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 10:48 ET, Morgan Chen reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-08 10:48 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-08 10:53 ET:** Jamie Christian accepted ownership, assigned the ticket to Storage and File Services, and began diagnostics.
- **2026-07-08 11:05 ET:** Initial tests isolated the fault to the Storage service layer on or affecting FS01.
- **2026-07-08 11:42 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-08 11:54 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-Disk`
2. `Get-PhysicalDisk`
3. `Get-ClusterResource`
4. `Get-WinEvent -LogName System -MaxEvents 200 | Where-Object Id -in 129,153,157`

### Diagnostic Findings

- `Get-Disk` → Command output matched the expected post-remediation configuration.
- `Get-PhysicalDisk` → Command output matched the expected post-remediation configuration.
- `Get-ClusterResource` → Command output matched the expected post-remediation configuration.
- `Get-WinEvent -LogName System -MaxEvents 200 | Where-Object Id -in 129,153,157` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.

The collected evidence showed that **A transient SAN path failure caused Windows to protectively offline the clustered disk.**

## Root Cause

A transient SAN path failure caused Windows to protectively offline the clustered disk.
...
Restored multipath connectivity, validated all paths, brought the disk online through cluster management, and ran filesystem checks.
...
- `SYS-0034-01-initial-alert.txt` — original user or monitoring report
- `SYS-0034-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0034-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-079 – Recover Offline Storage**.
...
Resolved SYS-0034 after isolating the failure to the Storage service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
