# SYS-0007: Nightly VM backup failed
...
| Opened | 2026-07-02 19:59 ET |
| First response | 2026-07-02 20:04 ET |
| Resolved | 2026-07-02 21:12 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-054 – Troubleshoot Backup Job Failures |

## Executive Summary

Nightly VM backup failed affected Marketing operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 19:59 ET, Avery Davis reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-02 19:59 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-02 20:04 ET:** Jamie Christian accepted ownership, assigned the ticket to Backup and Recovery, and began diagnostics.
- **2026-07-02 20:16 ET:** Initial tests isolated the fault to the Backup service layer on or affecting MON01.
- **2026-07-02 21:00 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-02 21:12 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-WinEvent -LogName Application -MaxEvents 200 | Where-Object Message -match 'backup'`
2. `Get-Volume`
3. `Get-VMSnapshot`

### Diagnostic Findings

- `Get-WinEvent -LogName Application -MaxEvents 200 | Where-Object Message -match 'backup'` → Relevant event sequence confirmed the root cause; no repeat error appeared after correction.
- `Get-Volume` → Capacity returned to a healthy operating range and remained below the warning threshold.
- `Get-VMSnapshot` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The backup repository reached its capacity threshold because retention pruning had failed.**

## Root Cause

The backup repository reached its capacity threshold because retention pruning had failed.
...
Removed expired restore points under retention policy, repaired the prune job, reran the backup, and performed a file-level restore test.
...
- `SYS-0007-01-initial-alert.txt` — original user or monitoring report
- `SYS-0007-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0007-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-054 – Troubleshoot Backup Job Failures**.
...
Resolved SYS-0007 after isolating the failure to the Backup service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
