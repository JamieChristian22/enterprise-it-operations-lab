# SYS-0079: Backup encryption key warning
...
| Opened | 2026-07-17 19:13 ET |
| First response | 2026-07-17 19:18 ET |
| Resolved | 2026-07-17 20:26 ET |
| SLA target | 15-minute response / 4-hour restoration |
| SLA result | Met |
| Closure code | Solved – configuration or service restoration |
| Related knowledge article | KB-060 – Protect Backup Encryption Keys |

## Executive Summary

Backup encryption key warning affected Marketing operations. The incident was investigated using documented, least-privilege administrative procedures. Service was restored without data loss, the underlying cause was corrected, and validation was completed with the requester or service owner.

## User or Monitoring Report

At 19:13 ET, Sydney Clark reported that the affected service was unavailable or behaving incorrectly. The issue was reproducible and had a confirmed business impact. No emergency workaround had been applied before the ticket reached Systems Administration.
...
- **2026-07-17 19:13 ET:** Ticket created and impact confirmed with requester or monitoring source.
- **2026-07-17 19:18 ET:** Jamie Christian accepted ownership, assigned the ticket to Backup and Recovery, and began diagnostics.
- **2026-07-17 19:30 ET:** Initial tests isolated the fault to the Backup service layer on or affecting MON01.
- **2026-07-17 20:14 ET:** Corrective action completed under the documented standard operating procedure.
- **2026-07-17 20:26 ET:** Technical validation and requester confirmation completed; ticket resolved within SLA.

## Diagnostic Procedure

1. `Get-ChildItem Cert:\LocalMachine\My | Sort NotAfter`
2. `Get-FileHash D:\Escrow\backup-key-export.pfx`

### Diagnostic Findings

- `Get-ChildItem Cert:\LocalMachine\My | Sort NotAfter` → Command output matched the expected post-remediation configuration.
- `Get-FileHash D:\Escrow\backup-key-export.pfx` → Command output matched the expected post-remediation configuration.

The collected evidence showed that **The recovery escrow certificate was due to expire within 14 days.**

## Root Cause

The recovery escrow certificate was due to expire within 14 days.
...
Issued and deployed a replacement certificate, exported the recovery key to approved escrow, and verified decryptability on an isolated restore.
...
- `SYS-0079-01-initial-alert.txt` — original user or monitoring report
- `SYS-0079-02-diagnostics.txt` — sanitized command results and event details
- `SYS-0079-03-validation.txt` — post-resolution validation record
...
- Linked the incident to **KB-060 – Protect Backup Encryption Keys**.
...
Resolved SYS-0079 after isolating the failure to the Backup service layer. Corrected the root cause, validated the original workflow, reviewed post-change logs, and obtained requester or service-owner confirmation. SLA met; no data loss or unresolved security exception.
