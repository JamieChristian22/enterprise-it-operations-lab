# KB-035: Validate a Backup Restore
...
Provide a repeatable, auditable procedure to diagnose and resolve validate a backup restore in the Contoso Systems Administration Lab.
...
- A restore is required after deletion or corruption
- A scheduled recovery test is due
- Backup integrity must be proven


## Likely causes

- Operational recovery request
- Quarterly resilience test
...
```text
Get-FileHash D:\RestoreTest\finance-export.csv -Algorithm SHA256
```
...
```text
Test-Path D:\RestoreTest\finance-export.csv
```
...
```text
Get-Item D:\RestoreTest\finance-export.csv | Select Length,LastWriteTime
```
...
- Restored object opens successfully
- Hash or application-level validation matches expectation
- Restore time meets the RTO
