# KB-002: Diagnose DNS Name Resolution Failures
...
Provide a repeatable, auditable procedure to diagnose and resolve dns name resolution failures in the Contoso Systems Administration Lab.
...
- Hostnames fail while IP connectivity works
- Applications report server not found
- Internal names resolve incorrectly or intermittently


## Likely causes

- Incorrect DNS server assignment
- Stale client cache or DNS record
- Broken conditional forwarder, zone replication, or firewall path
...
```text
Resolve-DnsName fileserver01.corp.contoso.com
```
...
```text
nslookup fileserver01.corp.contoso.com
```
...
```text
Get-DnsServerResourceRecord -ZoneName corp.contoso.com -Name fileserver01
```
...
- Resolve the name from two clients
- Confirm the returned IP matches the CMDB
- Verify TCP and UDP 53 reachability
