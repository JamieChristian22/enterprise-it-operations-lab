# KB-036: Troubleshoot Monitoring Agent Failures
...
Provide a repeatable, auditable procedure to diagnose and resolve monitoring agent failures in the Contoso Systems Administration Lab.
...
- Host is stale or unknown in monitoring
- Metrics or logs stop arriving
- Agent service repeatedly fails
...
```text
Get-Service ContosoMonitorAgent
```
...
```text
Test-NetConnection monitor01.contoso.com -Port 443
```
...
```text
systemctl status contoso-monitor-agent
```
...
```text
journalctl -u contoso-monitor-agent --since "1 hour ago"
```
...
- Host reports current heartbeat
- Metrics and logs resume
- Agent remains healthy for 15 minutes
