# KB-017: Troubleshoot Linux SSH Connection Failures
...
Provide a repeatable, auditable procedure to diagnose and resolve linux ssh connection failures in the Contoso Systems Administration Lab.
...
- SSH times out, refuses connection, or rejects credentials
- Key authentication stops working
- Only one account is affected


## Likely causes

- sshd service or firewall issue
- Incorrect permissions on .ssh or authorized_keys
- Account lockout, shell, or access directive
...
```text
sudo ls -ld /home/jcarter/.ssh /home/jcarter/.ssh/authorized_keys
```
...
- SSH succeeds with the approved method
- Failed-login logs stop recurring
- Root and password access remain aligned with policy
