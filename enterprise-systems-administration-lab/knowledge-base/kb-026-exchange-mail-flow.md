# KB-026: Troubleshoot Exchange Online Mail Flow
...
Provide a repeatable, auditable procedure to diagnose and resolve exchange online mail flow in the Contoso Systems Administration Lab.
...
- Message is delayed, rejected, quarantined, or missing
- External delivery fails
- Only one sender, recipient, or domain is affected


## Likely causes

- Transport rule, connector, or anti-spam action
- Invalid DNS records or recipient
- Service incident or mailbox restriction
...
```text
Get-MessageTrace -SenderAddress jcarter@contoso.com -StartDate (Get-Date).AddHours(-24) -EndDate (Get-Date)
```
...
- Message trace shows Delivered
- Recipient confirms receipt
- Root cause and message ID are recorded
