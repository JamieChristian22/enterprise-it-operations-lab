# KB-029: Reset Microsoft Entra MFA Methods Safely

| Field | Value |
|---|---|
| Service | Identity / Microsoft Entra |
...
Provide a repeatable, auditable procedure to diagnose and resolve microsoft entra mfa methods safely in the Contoso Systems Administration Lab.
...
- User lost or replaced an authentication device
- Authentication method is unusable
- User is trapped in an MFA loop
...
- Lost phone
- Changed number
- Corrupt Authenticator registration
...
```text
Get-MgUserAuthenticationMethod -UserId jcarter@contoso.com
```
...
```text
Remove-MgUserAuthenticationMicrosoftAuthenticatorMethod -UserId jcarter@contoso.com -MicrosoftAuthenticatorAuthenticationMethodId 00000000-0000-0000-0000-000000000001
```
...
```text
New-MgUserAuthenticationTemporaryAccessPassMethod -UserId jcarter@contoso.com -BodyParameter @{lifetimeInMinutes=60;isUsableOnce=$true}
```
...
- User registers two approved methods
- A new sign-in satisfies MFA
- Temporary access pass is expired or removed
...
```text
INC-260829 — Reset Microsoft Entra MFA Methods Safely
...
Validation: User registers two approved methods; A new sign-in satisfies MFA.
