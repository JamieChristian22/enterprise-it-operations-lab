# KB-022: Troubleshoot Linux cron Job Failures
...
Provide a repeatable, auditable procedure to diagnose and resolve linux cron job failures in the Contoso Systems Administration Lab.
...
- Task works interactively but not in cron
- Expected output or file is missing


## Likely causes

- Minimal cron PATH or environment
- Incorrect user, permissions, or schedule
- Script depends on relative paths or interactive input
...
```text
sudo runuser -u backupsvc -- /opt/jobs/backup.sh
```
...
- Manual execution as the service account succeeds
- Next scheduled run completes
- Logs capture exit code and duration
