...
param(
    ...
    [string]$OutputPath = ".\output\backup-freshness.json"
)
...
$latest = Get-ChildItem $Path -File -Recurse |
...
$result = [pscustomobject]@{
    ...
    AgeHours = if ($latest) {
        [math]::Round(((Get-Date) - $latest.LastWriteTime).TotalHours, 2)
    } else {
        $null
    }
    Passed = if ($latest) {
        ((Get-Date) - $latest.LastWriteTime).TotalHours -le $MaximumAgeHours
    } else {
        $false
    }
}
